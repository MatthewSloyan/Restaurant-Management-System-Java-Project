...
public class RestaurantManagementSystem implements CleaningExpense, Bonus{
	...
	public static void main(String[] args) throws IOException
	{
		...
		// Record Layout to input login file
		String loginFirstName;
		String loginLastName;
		String loginPassword;
		char loginJob;
		
		//general variables for login/create employee system
		int choice;
		...
		while (choice != SENTINEL) // Continue until Q/Exit sentinel encountered
		{
			switch (choice)
			{
				...
				case 'C':
					...
					}while (!(job == 'M')&&(!(job == 'B'))&&(!(job == 'C'))&&(!(job == 'J'))&&(!(job == 'W')));//checks for correct input
				
					//creates employee object using anonymous class, as Employee is abstract, also implements abstract method for system
			...
			} // login or create switch 

			System.out.println();
			choice = showLoginSignUp(); // subsequent read
		} // while
 	...
 	public static int showManagerMenu()
    {
 		...
 		do
		{
            ...
            if (menuInput > 6 || menuInput < 0) {
				...
				(!(menuInput == 4))&&(!(menuInput == 5)&&(!(menuInput == 6))));//checks for correct input
 		
        return menuInput;
    }
 	
	// job specific selection methods, e.g manager, chef etc.
	//===================================================================
 	...
 	{
 		//print Mangager employee file and log complaints
		PrintWriter outFileM = new PrintWriter("manager.dat");
		PrintWriter outFileMC = new PrintWriter("complaints.dat");
		
		final int MENU_SENTINEL = 0; //sentinel to exit class menu
		...
		//manager variables
 		...
 		menuInput = showManagerMenu(); //initial read
	        ...
	        switch (menuInput)
			{
				case 1:
					System.out.println("What is your weekly wage: ");
			 		...
			 		m1.setWeeklyWage(weeklyWage); //set method
					break;
				case 2:
				...
				case 3:
					...
					for(int i = 1; i <= numOfComplaints; i++) {
						...
						complaint = console.nextLine(); // read in complaint string
 	...
 	public static void bartenderSelection(String loginFirstName, String loginLastName, String loginPassword, char loginJob) throws FileNotFoundException 
 	{
        ...
        {
	        ArrayList<Drinks> drinksList = new ArrayList<>(); //array list using Drinks Class object
	        
	        //loop for number of drinks served
	        for(int i = 1; i <= barTenderDrinksServed; i++) 
	        {
				...
				}while (!(menuInput == 0)&&!(menuInput == 1)&&!(menuInput == 2)&&(!(menuInput == 3))&&(!(menuInput == 4)));//checks for correct input
	...
	//===================================================================
 	...
 	public static void janitorSelection(String loginFirstName, String loginLastName, String loginPassword, char loginJob) throws FileNotFoundException 
 	{
		...
		if (cleaningCostInput == 'Y') {
			System.out.println("The Total cleaning cost for today is " + j1.calculateCleaningCost(roomsCleared, COST_PER_ROOM) + "\n");
		}
	...
	//===================================================================
 	...
 	{	
		...
		System.out.println("\nName		    Pw      Job    Details");
 		...
 		// Main while loop employee file Input until EOF
 	...
 	public static void calculateFloorStaffWages(double hours, double overTime) 
 	{
        ...
        if(wagesInput == 'Y') {
	        //functional interface using lambdas to calculate wages
	        ...
	        //call the calculate wages method
